...
const AutresEmissionsList = ["Autres Emissions indirectes"];

function AutresEmissions() {
  return (
    <Grid>
      {AutresEmissionsList.map((deplacement, index) => (
        <Box key={index} p={2} bgcolor={"#F0F2F7"} mb={2} borderRadius={4}>
          <Grid container justifyContent="space-between" alignItems="center">
            ...
            <Grid item xs={12} md={3} sx={{ textAlign: "center" }}>
              <Button disabled style={Styles.ajouterActiviteButton}>Disabled</Button>
            </Grid>
          </Grid>
        </Box>
      ))}
